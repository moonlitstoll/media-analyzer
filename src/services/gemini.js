...
**[1. 전수 발화 및 공백 제로 원칙 (No Omission & Gap Hunting)]**
- **동적 갭 필러 (Dynamic Gap Filler)**: 이전 문장 종료와 다음 문장 시작 사이의 간격(Gap)이 3초를 초과하는 경우, 해당 구간의 오디오 파형을 정밀 재탐색하여 반드시 텍스트(랩, 웅얼거림, 추임새 등)를 추출하십시오.
- **광대역 화자 적응 (Speaker Continuity)**: 화자의 톤이나 피치가 급격히 변화해도 이를 노이즈로 처리하지 말고 '연속된 발화'로 인식하여 중단 없이 분석하십시오.
- **반복 요약 금지**: 동일 문장이 반복되어도 매번 새로운 타임라인과 함께 출력하십시오.

**[2. 안티 고스트 가드레일 (Zero Blank Policy)]**
- **빈 카드 생성 금지**: 타임라인이 잡혔으나 텍스트가 없는 경우(Ghost Card)는 치명적 오류입니다.
- **Fallback 텍스트 강제**: 어떤 이유로든 해석이 불가능한 발화 구간이라면, 공백("") 대신 **(가사/대사 분석 불가 구간)** 또는 **(Inaudible)**이라는 텍스트를 넣어 카드의 형태를 반드시 유지하십시오.
- **전수 분석 연동**: 위의 가이드 텍스트 구간에 대해서도 반드시 Translation 및 6대 원칙 기반 Word Analysis를 채워 넣으십시오.
