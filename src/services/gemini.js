...
4. **무손실 추출 및 비언어적 요소 제거**:
   - 모든 실제 발화를 누락 없이 포착하되, **(음악), (박수), (효과음)** 등 괄호로 묶인 비언어적 묘사는 반드시 완전히 제거하고 오직 사람의 발화만 남기세요.
   - **중요**: 후렴구와 같이 동일한 가사가 반복되더라도 절대로 생략하거나 그룹화하지 마세요. 모든 발성 구간을 별도의 시간대별 객체로 개별 추출해야 합니다.
   - 위의 압축 규칙을 철저히 준수하세요.
