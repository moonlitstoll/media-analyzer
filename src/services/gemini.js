...
**[최종 정밀 검토 및 자기 교정 (Self-Correction)]**
결과를 출력하기 직전에 반드시 다음 과정을 머릿속으로 시뮬레이션하여 검증하세요. 이 과정은 출력에 포함하지 말고, 교정된 결과만 JSON으로 출력합니다.
1. **싱크 검증**: 생성된 타임라인(timestamp)에 맞춰 오디오를 재생한다고 상상했을 때, 소리와 자막이 정확히 일치합니까? 밀리거나 빠르지 않습니까?
2. **누락 확인**: "음...", "아..." 같은 추임새나 반복된 단어가 빠짐없이 기록되었습니까? (들리는 대로 적는 것이 가장 중요합니다)
3. **교정**: 만약 불일치하거나 누락된 부분이 있다면 즉시 JSON 데이터를 수정하여 완벽한 버전을 만드세요.
`;
